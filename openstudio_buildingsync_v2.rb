  ...
  def hash_to_xml(h)
    ...
    if(h.is_a?(Hash))
      if(h.keys.any? {|x| allowable_keys.include?(x) })
        ...
        if(h.key?(:attributes) || h.key?('attributes'))
        ...
        if(h.key?(:children) || h.key?('children'))
          # puts "Has children."
              ...
              if(child.key? valuekey)
                if(child[valuekey].is_a?(Array))
                  ...
                  child[valuekey].each do |c|
                    ...
                    self.mostRecentElement = mostRecentElement.add_element(k.to_s)
                    hash_to_xml(c)
                    puts 'Assigning to parent.'
                    self.mostRecentElement = mostRecentElement.parent
                  end
        ...
        if(h.values[0].key? :type)
          puts 'Writing ' + h.values[0][:type] + ' element.'
          self.mostRecentElement = mostRecentElement.add_element(h.values[0][:type]) # this is a string
        else
          puts 'Writing ' + h.keys[0].to_s + ' element.'
          self.mostRecentElement = mostRecentElement.add_element(h.keys[0].to_s)
        end
...
class SimpleElement
  ...
  def post_initialize(args)
    ...
    if(!args.nil?)
      if(args.is_a?(Hash))
        if(args.key? :children)
          bulk_children(args[:children])
        elsif(args.key? :attributes)
